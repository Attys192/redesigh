...
export default function StructurePage() {
  ...
  useEffect(() => {
    fetch('http://localhost:3005/api/departments')
      .then((res) => {
        ...
        uniqueNames.map(async (name) => {
          try {
            const res = await fetch(
              `http://localhost:3005/api/staff/find?name=${encodeURIComponent(name!.split(' ')[0])}`
            );
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-blue-600 to-blue-800 p-8 text-white shadow-2xl shadow-blue-200 md:p-12">
        ...
        <section key={category} className="space-y-6">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {items.map((dept) => (
              <div
                key={dept.id}
                // h-full гарантирует, что карточки в одном ряду будут одинаковой высоты
              ...
              >
                <div className="absolute right-0 top-0 -mr-12 -mt-12 h-24 w-24 rounded-bl-full bg-blue-50/50 transition-colors group-hover:bg-blue-100/50"></div>

                {/* 1. БЛОК ЗАГОЛОВКА: Выделен отдельно с min-h-[5rem] (хватает на 3-4 строки). 
                    Это резервирует одинаковое место в каждой карточке, чтобы аватары были на одной линии */}
                <div className="min-h-[5rem] md:min-h-[5.5rem]">
                  <h3 className="pr-8 text-lg font-bold leading-tight text-slate-800">{dept.name}</h3>
                </div>

                {/* 2. БЛОК АВАТАРА: Занимает оставшееся место (flex-grow) */}
                <div className="flex-grow space-y-4">
                  {dept.headName ? (
                    staffMap[dept.headName] ? (
                      <Link
                        href={`/student/staff/${staffMap[dept.headName].id}`}
                        className="grid grid-cols-[8rem_minmax(0,1fr)] items-start gap-4 rounded-2xl p-3 text-blue-600 transition-colors hover:bg-blue-50 -ml-3"
                      >
                        <StaffAvatar
                      ...
                      <div className="grid grid-cols-[8rem_minmax(0,1fr)] items-start gap-4 rounded-2xl p-3 text-slate-400 -ml-3">
                        <StaffAvatar
                          name={dept.headName}
                          className="flex h-32 w-32 rounded-2xl border border-slate-200 bg-white p-2"
                          imageClassName="object-contain"
                    ...
                    /* Пустое состояние (Например, для Студенческого совета) */
                    <div className="grid grid-cols-[8rem_minmax(0,1fr)] items-start gap-4 rounded-2xl p-3 text-slate-300 -ml-3 opacity-70 cursor-default">
                ...
                {/* 3. БЛОК КОНТАКТОВ: mt-auto жестко прибивает этот блок к самому низу карточки */}
                <div className="mt-auto flex flex-col space-y-3 pt-2">
                  ...
                  {dept.email && (
                    <div className="text-xs text-slate-500">
                      ...
                      <a href={`mailto:${dept.email}`} className="transition-colors hover:text-blue-600">
                        {dept.email}
                      </a>
                    </div>
                  )}
                  {dept.website && (
                    <div className="flex items-center gap-3 text-xs text-slate-500">
                      ...
                      <a
                        ...
                        className="underline decoration-blue-100 underline-offset-4 transition-colors hover:text-blue-600"
                      >
                        {dept.website}
                      </a>
                    </div>
