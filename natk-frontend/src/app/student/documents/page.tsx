...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    fetch('http://localhost:3005/api/documents')
      .then((res) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-3xl p-8 md:p-12 text-white shadow-2xl shadow-blue-200 relative overflow-hidden">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 space-y-4">
        <div className="w-full">
          <input
            ...
            className="w-full px-4 py-3 bg-slate-50 border-none rounded-2xl focus:ring-2 focus:ring-blue-500 transition-all text-slate-700 placeholder:text-slate-400"
            value={searchQuery}
        ...
        {filteredDocs.map((doc) => (
          ...
          >
            <div className="flex items-start justify-between">
              ...
              <div className="bg-slate-50 px-3 py-1 rounded-full text-[10px] font-black text-slate-400 uppercase tracking-wider group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors">
                {doc.fileUrl.split('.').pop()?.toUpperCase() || 'DOC'}
              </div>
            </div>

            <div className="flex-1">
              <h3 className="text-slate-800 font-bold leading-snug group-hover:text-blue-600 transition-colors line-clamp-3">
                {doc.title}
              </h3>
            ...
            <div className="pt-4 border-t border-slate-50 flex items-center justify-between text-blue-600 font-bold text-sm">
