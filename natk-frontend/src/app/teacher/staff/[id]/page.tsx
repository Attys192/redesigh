    ...
    const fetchStaffData = async () => {
      try {
        ...
        const scheduleRes = await fetch(
          `${apiUrl}/api/schedule?teacher=${encodeURIComponent(lastName)}`
        );
        if (scheduleRes.ok) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4">
        <Link
          href="/teacher"
          className="inline-flex items-center gap-2 text-slate-600 hover:text-amber-700 font-medium transition-colors"
        >
          <ChevronRight size={16} className="rotate-180" />
          Назад в кабинет
        </Link>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-6">
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {staff.positions.map((pos) => (
                  <span
                    key={pos.id}
                    className="bg-amber-50 text-amber-700 px-3 py-1 rounded-lg text-sm font-medium"
                  >
                    {pos.positionName}
                  </span>
                ))}
                ...
                <div className="border-t border-slate-200 pt-5 space-y-3">
                  {contactInfo.phone && (
                    <a
                      ...
                      className="flex items-center gap-3 text-slate-700 hover:text-amber-700 transition-colors"
                    >
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {staff.achievements.map((imgUrl, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="aspect-[3/4] rounded-xl overflow-hidden border-2 border-slate-200 hover:border-amber-600 transition-all group relative"
                    >
                      <img
                        src={imgUrl}
                      ...
                      />
                      <div className="absolute inset-0 bg-amber-600/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white font-medium">
                        Увеличить
                      </div>
                    </a>
                  ))}
...
function BioCard({ section }: { section: BioSection }) {
  ...
  if (isContact) {
    ...
    return (
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
        <h3 className="text-xl font-bold text-amber-700 mb-4">Контактная информация</h3>
        <div className="h-px bg-slate-100 mb-4" />
        <div className="flex flex-wrap gap-3 mb-3">
          {phoneMatch && (
            <a
              href={phoneMatch[1]}
              className="inline-flex items-center gap-2 bg-amber-50 hover:bg-amber-100 text-amber-700 font-medium px-4 py-2 rounded-xl transition-colors"
            >
              <Phone size={16} />
              {phoneMatch[2]}
            </a>
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
      <h3 className="text-xl font-bold text-amber-700 mb-4">{section.title}</h3>
      <div className="h-px bg-slate-100 mb-4" />
      <div
